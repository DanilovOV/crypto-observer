...
let tabId
let tabType
const tabsSyncWorker = new SharedWorker(
	new URL('../workers/tabsSyncWorker', import.meta.url),
	{ type: 'module' }
)

let tabInitResolve
let tabInitPromise = new Promise((resolve) => {
	tabInitResolve = resolve
})
...
export const getTabType = callAfterInit(() => tabType)

export function postMessage({ header, message }) {
	callAfterInit(async ({ header, message }) => {
...
export const waitWorkerData = callAfterInit(async ({ responceHeader }) => {
	let result
	let resultReceived
